...
class BaseDataset(Dataset):
    ...
    def __init__(
        ...
        self.stride = stride
        self.hyp = hyp  # Training hyperparameters.
        self.pad = pad
        if self.rect:
    ...
    def load_image(self, i, rect_mode=True):
        ...
        if im is None:  # not cached in RAM
            if fn.exists():  # load npy
                ...
                im = cv2.imread(f)  # BGR
                if self.hyp.ch > 3:  # Multimodal input.
                    ir_im = cv2.imread(ir)
                    if ir_im is None:
                        raise FileNotFoundError(f"Paired modality image not found: {ir}")
                    im = cv2.merge((ir_im, im))  # Merge the paired modalities.
            if im is None:
