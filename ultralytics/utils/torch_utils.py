...
def model_info(model, detailed=False, verbose=True, imgsz=640):
    ...
    flops = get_flops(model, imgsz) # FLOPs.
...
def get_flops(model, imgsz=640):
    ...
    try:
        ...
        try:
            # Infer the input channel count expected by the model.
            input_channels = 6  # Default to the multimodal setting used in this repository.
            # If the model exposes a different input channel count, inspect the first layer.
            first_layer = list(model.modules())[0]
            if isinstance(first_layer, torch.nn.Conv2d):
                input_channels = first_layer.in_channels
            # Prepare the input tensor.
            # stride = max(int(model.stride.max()), 32) if hasattr(model, 'stride') else 32  # Maximum stride.
            stride = 640
            im = torch.zeros((1, input_channels, stride, stride), device=next(model.parameters()).device)
            # Compute MACs with thop.profile.
